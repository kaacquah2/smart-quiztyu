...
async function checkQuizCoverage() {
  try {
    console.log('Checking quiz coverage for all programs and courses...\n')
    ...
    quizzes.forEach(quiz => {
      quizQuestionCounts[quiz.id] = quiz._count.questions
    })

    // Track programs with insufficient quiz coverage
    const programsWithInsufficientCoverage = []

    // Check each program
    for (const program of programs) {
      const coursesWithInsufficientQuestions = []
      
      // Check each course in the program
      for (const year of program.years) {
        for (const semester of year.semesters) {
          for (const course of semester.courses) {
            const questionCount = quizQuestionCounts[course.id] || 0
            
            if (questionCount < 10) {
              coursesWithInsufficientQuestions.push({
                courseId: course.id,
      ...
      if (coursesWithInsufficientQuestions.length > 0) {
        programsWithInsufficientCoverage.push({
          programId: program.id,
          programTitle: program.title,
          courses: coursesWithInsufficientQuestions
        })
      }
    }

    // Display results
    if (programsWithInsufficientCoverage.length === 0) {
      console.log('✅ All programs have courses with at least 10 quiz questions!')
    } else {
      console.log('❌ Programs with courses that have fewer than 10 quiz questions:\n')
      
      for (const program of programsWithInsufficientCoverage) {
        console.log(`📚 ${program.programTitle} (${program.programId})`)
        console.log(`   Courses with insufficient questions: ${program.courses.length}\n`)
        
        for (const course of program.courses) {
          const status = course.questionCount === 0 ? '❌' : '⚠️'
          console.log(`   ${status} ${course.courseTitle}`)
          console.log(`      Course ID: ${course.courseId}`)
          console.log(`      Questions: ${course.questionCount}/10`)
      ...
      // Summary statistics
      const totalInsufficientCourses = programsWithInsufficientCoverage.reduce(
      ...
      console.log(`   Programs with insufficient coverage: ${programsWithInsufficientCoverage.length}`)
      console.log(`   Total courses: ${totalCourses}`)
      console.log(`   Courses with insufficient questions: ${totalInsufficientCourses}`)
      console.log(`   Coverage percentage: ${Math.round(((totalCourses - totalInsufficientCourses) / totalCourses) * 100)}%`)
    }

  } catch (error) {
    console.error('Error checking quiz coverage:', error)
    throw error
  } finally {
...
checkQuizCoverage()
  .then(() => {
    console.log('\nQuiz coverage check completed!')
    process.exit(0)
  })
  .catch((error) => {
    console.error('Failed to check quiz coverage:', error)
    process.exit(1)
  }) 