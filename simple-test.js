...
async function simpleTest() {
  ...
  try {
    ...
    try {
      const ruleBasedService = require('./lib/rule-based-recommendations-service')
      console.log('  ✅ Rule-based recommendations service loaded')
    } catch (error) {
      console.log('  ❌ Rule-based recommendations service failed to load:', error.message)
    }
    
    try {
      const deepseekService = require('./lib/deepseek-recommendations-service')
      console.log('  ✅ DeepSeek recommendations service loaded')
    } catch (error) {
      console.log('  ❌ DeepSeek recommendations service failed to load:', error.message)
    }
    
    try {
      const geminiService = require('./lib/gemini-study-plan-service')
      console.log('  ✅ Gemini study plan service loaded')
    } catch (error) {
      console.log('  ❌ Gemini study plan service failed to load:', error.message)
    }
    ...
    try {
      ...
      console.log('  ❌ Cache service failed to load:', error.message)
    }
    ...
    try {
      ...
      console.log('  ❌ Logger service failed to load:', error.message)
    }
    ...
    try {
      const { generateRuleBasedRecommendations } = require('./lib/rule-based-recommendations-service')
      ...
      console.log('  ❌ Rule-based recommendations failed:', error.message)
    }
    
    // Test 5: Basic study plan generation
    console.log('\n📚 Testing Study Plan Generation...')
    try {
      const { generateGeminiStudyPlan } = require('./lib/gemini-study-plan-service')
      ...
      console.log('  ❌ Study plan generation failed:', error.message)
    }
    ...
    console.log('\n📈 Next Steps:')
    console.log('1. Run the full test suite: node test-personalization-system.js')
